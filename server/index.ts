...
app.get("/api/stats/:deviceId", async (request, response) => {
  const displayName =
    typeof request.query.displayName === "string"
...
async function emitRoomUpdate(roomCode: string) {
  ...
  for (const roomSocket of sockets) {
    const roomState = getRoomForDevice(
      roomCode,
      roomSocket.data.deviceId ?? "",
    );
...
async function emitVoiceSignal(payload: VoiceSignalPayload) {
  ...
  const targetSocket = sockets.find(
    (roomSocket) => roomSocket.data.deviceId === payload.toDeviceId,
  );
...
io.on("connection", (socket) => {
  ...
  socket.on("room:reconnect", async (payload, callback) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to reconnect to room.";
      callback({ error: message });
    }
  });
  ...
  socket.on("cell:submit", async (payload, callback) => {
    try {
      ...
      if (result.room.phase === "finished") {
        ...
        for (const roomSocket of sockets) {
          const roomState = getRoomForDevice(
            ...
            roomSocket.data.deviceId ?? payload.deviceId,
          );
          if (roomState) {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to submit value.";
      socket.emit("room:error", message);
  ...
  socket.on("voice:audio", async (payload) => {
    try {
      ...
      const sockets = await io.in(payload.roomCode).fetchSockets();
      const recipients = sockets.filter(
        (roomSocket) => roomSocket.data.deviceId !== payload.fromDeviceId,
      );
      ...
      const message =
        error instanceof Error ? error.message : "Unable to relay voice audio.";
      socket.emit("room:error", message);
  ...
  socket.on("notes:update", async (payload, callback) => {
    try {
      ...
      const updated = await updateNotes(
        payload.roomCode,
        payload.deviceId,
        payload.notes,
      );
      ...
      const message =
        error instanceof Error ? error.message : "Unable to update notes.";
      socket.emit("room:error", message);
      callback?.({ error: message });
    }
  });
